...
{	
	...
	// 형태소 분석결과를 어절단위로 인식
	for ( int wrd_idx=0, ejl_idx=0; words->one[wrd_idx].raw[0]!='\0'; wrd_idx++, ejl_idx++ ) 
	{
		for ( int i=1; (words->one[wrd_idx+i].pos[0]==' ')||(words->one[wrd_idx+i].pos[0]=='+'); i++ ) 
		{
	...
	for ( wrd_idx=wrd_idx-1, ejl_idx=0; wrd_idx>=0; wrd_idx-- )
	{
		...
		for ( int i=0; (head[EOJEOL][wrd_idx]==head[EOJEOL][wrd_idx-i])&&(wrd_idx>=i); i++ )
		{ 
...
{	
	...
	for ( int i=0; words->one[i].raw[0]!='\0'; i++ ) 
	{
		for ( int j=1; (words->one[i+j].pos[0]==' ')||(words->one[i+j].pos[0]=='+'); j++ ) 
		{
