...
aPhrase *cStaticRule::getParent( char *pos_tagged_word, short word_idx, Phrases *phrases )
{
	...
	{
		for ( int i=0; strcmp(m_pos2node[i].pos,pos)&&(m_pos2node[i].pos[0]!='\0'); i++ );  // 내용/기능(구문)
		if( m_pos2node[i].pos[0]=='\0' ) { printf("\nERROR(cStaticRule::initPhrase): 구문초기화오류(%s/%s)\n",raw,pos); return false; }	
...
void cStaticRule::segment( int idx[2][MAX_UPS_CNT], aWord spacingUnit )
{
	...
	for ( bool prn=false, brk=false, brc=false, dqut=false, squt=false; (i<=(int)strlen(spacingUnit.raw)); i++ )
	{
...
void cStaticRule::getSynUnits( Words *words, Words *spacingUnits )
{
	...
	for( int i=0, j=0, idx[2][MAX_UPS_CNT]; spacingUnits->one[i].raw[0]!='\0'; i++, j++ )
	{ 
		// 기본 단위로 분할: 띄어쓰기, 어절간괄호, 마침표
		segment( idx, spacingUnits->one[i] );
		for ( int k=0; idx[RAW][k]<idx[RAW][k+1]; j++, k++ )
		{ 
...
void cStaticRule::checkWord( char **arg_raw, char **arg_pos )
{
	...
	// 공백 및 + 선별 제거
	for ( int i=0, j=0; (raw[i]!='\0'); i++, j++ ) { if ( (raw[i]==' ') ){ j--; } else { raw[j]=raw[i]; } } 
	if ( (raw[j]!='\0') ) { raw[j]='\0'; }
...
{
	char symbol; for ( int i=0; (raw[i]==' ')||(raw[i]=='+'); i++ ); 
...
cnvrt cStaticRule::m_pos2node[MAX_POS_CNT] =	
{
	...
	{ "X",	"_CMP",	"JKC"	},		// 보격   조사[보어]
	{ "X",	"_MOD",	"JKG"	},		// 관형격 조사
	{ "X",	"_OBJ",	"JKO"	},		// 목적격 조사[목적어]
	{ "X",	"_AJT",	"JKB"	},		// 부사격 조사[부사어]
	{ "X",	"_INT",	"JKV"	},		// 호격   조사
	{ "X",	"_CMP",	"JKQ"	},		// 인용격 조사
	{ "X",	"",		"JX"	},		// 보조사     [미지격]
	{ "X",	"_CNJ",	"JC"	},		// 접속격 조사[접속 또는 연결]
...
#define		DERV(l,r)  !strncmp(r->cont.pos,"XS",strlen("XS"))||!strncmp(r->cont.pos,"VCP",strlen("VCP"))||!strncmp(r->cont.pos,"ETN",strlen("ETN"))
#define		INFL(l,r)  (r->cont.pos[0]=='J')||(r->cont.pos[0]=='E')
...
aPhrase* cStaticRule::setParent( aPhrase* parent, aPhrase* left, aPhrase* right )
{
	...
	// 파생(접미사)
	else if ( DERV(left,right) ) { memcpy(&(parent->cont),&(left->cont),sizeof(Lex)); parent->cont.syn[0] = right->cont.syn[0]; }
	// 굴절(조사/어미)
...
*/
	...
	if ( parent->cont.syn[0]=='S' ) { parent->cont.syn[1]='\0'; }
	// (NP (NP 소크라테스) (X +적))
	else if ((parent->cont.syn[0]=='X')&&(left->cont.syn[0]!= 'X') ){ strcpy(parent->cont.syn,left->cont.syn); }
	// 헌팅(hunting) (X (X +에서) (X +는))
	else if ( parent->cont.syn[0]=='X' ) { parent->cont.syn[1]='\0'; }
	// 
	// 구문_기능 표지 부착 예외: 서술격조사
//	else if ((parent->cont.syn[0]=='V')&&!strncmp(parent->cont.pos,"VC",strlen("VC")) ) { strcpy(parent->cont.syn,"VP"); }
/*	// <배식>이라는: (X_MOD 이라는)
	else if ( !strncmp(parent->cont.pos,"VCP",strlen("VCP")) ) { strcpy(parent->cont.syn,"X"); }
	// <배식>이라는: (VP_MOD <배식>이라는)
	else if ( !strncmp(right->cont.pos,"VCP",strlen("VCP")) ) { strcpy(parent->cont.syn,"VP"); }
	// 헌팅(hunting)하러: (X_MOD 하러)
	else if ( !strncmp(parent->cont.pos,"XSV",strlen("XSV")) ) { strcpy(parent->cont.syn,"X"); }
	// 헌팅(hunting)하러: (VP_MOD 헌팅(hunting)하러)
	else if ( !strncmp(right->cont.pos,"XSV",strlen("XSV")) ) { strcpy(parent->cont.syn,"VP"); }
*/	// 나머지는 P(phrase)부착
...
/******/
	if ( !strcmp(parent->cont.syn,"NP")&&!strcmp(left->cont.syn,"NP")&&!strcmp(right->cont.syn,"AP") ) 
	{ int q=0; }
	if ( !strcmp(parent->cont.raw,"+이") ) 
	{ int q=0; }
/******/
